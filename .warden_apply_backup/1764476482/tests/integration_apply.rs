...
fn test_truncation_allows_warden_ignore() {
    ...
    use warden_core::apply::types::ApplyOutcome;
    
    // Extractor will extract content normally
    let input = format!("{} src/ignored.rs {}\n// warden:ignore\n// ...\n{}", nabla(), nabla(), delta());
    let files = extractor::extract_files(&input).unwrap();
    
    // Validator should NOT report truncation error because of the ignore directive
    let manifest = Vec::new();
    let result = validator::validate(&manifest, &files);
    
    match result {
        ApplyOutcome::Success { .. } => {
            // This is the expected happy path: validation passes despite "// ..."
        },
        ApplyOutcome::ValidationFailure { errors, .. } => {
            // If it failed, verify it wasn't because of truncation
            let truncation_errors: Vec<_> = errors.iter()
                .filter(|e| e.contains("truncation") || e.contains("marker"))
                .collect();
                
            if !truncation_errors.is_empty() {
                panic!("warden:ignore should bypass truncation checks, but got: {:?}", truncation_errors);
            }
        }
