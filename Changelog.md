...
### Added
- **Agentic Stage**: Added `slopchop stage` command to explicitly manage the staging sandbox.
- **Nuclear Sync**: Added `apply --sync` flag to mirror the stage worktree to the repository root with safety protections.
- **Heuristic Nag**: Added advisory warnings in `slopchop check` when high edit volume (>3 files) is detected in the stage.
- **Modular CLI Handlers**: Refactored `handlers.rs` by extracting `stage_handlers.rs` to maintain codebase modularity and manage file sizes.

### Changed
- **Stage Workflow**: Shifted to a direct-edit sandbox model where agents work on a shadow worktree and synchronize changes back.
