...
fn parse_manifest_lines(block: &str) -> Result<Vec<ManifestEntry>> {
    ...
    for line in block.lines() {
        ...
        let final_path = path_raw.split_whitespace().next().unwrap_or(&path_raw).to_string();

        if !final_path.is_empty() {
            entries.push(ManifestEntry { path: final_path, operation: op });
        }
    }
    Ok(entries)
}
...
fn parse_operation(line: &str) -> (String, Operation) {
    ...
    if upper.contains("[NEW]") {
        (line.replace("[NEW]", "").replace("[new]", ""), Operation::New)
    } else if upper.contains("[DELETE]") {
        (line.replace("[DELETE]", "").replace("[delete]", ""), Operation::Delete)
    } else {
