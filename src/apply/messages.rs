...
pub fn print_outcome(outcome: &ApplyOutcome) {
    match outcome {
        ApplyOutcome::Success { written, deleted, backed_up } => {
            println!("{}", "? Apply successful!".green().bold());
            if *backed_up { println!("   (Backup created)"); }
            for file in written { println!("   {} {file}", "�".green()); }
            for file in deleted { println!("   {} {file}", "?".red()); }
...
pub fn format_ai_rejection(missing: &[String], errors: &[String]) -> String {
    ...
    if !missing.is_empty() {
        ...
        for f in missing { let _ = writeln!(msg, "- {f}"); }
    }
    if !errors.is_empty() {
        msg.push_str("VALIDATION ERRORS:\n");
        for e in errors { let _ = writeln!(msg, "- {e}"); }
    }
