...
impl FingerprintState {
    ...
    fn visit(&mut self, node: Node, source: &[u8], depth: usize) {
        ...
        let kind = node.kind();
        count_cfg_elements(self, kind);
        hash_node(self, kind, node, source, depth);
        visit_children(self, node, source, depth);
    }
...
fn count_cfg_elements(state: &mut FingerprintState, kind: &str) {
...
fn hash_node(state: &mut FingerprintState, kind: &str, node: Node, source: &[u8], depth: usize) {
    0xDEAD_BEEF_u64.wrapping_add(depth as u64).hash(&mut state.struct_hasher);
    kind.hash(&mut state.struct_hasher);

    if should_hash_text(kind, node, source) {
        if let Ok(text) = node.utf8_text(source) {
            text.hash(&mut state.struct_hasher);
        }
    }

    if CFG_NODES.contains(&kind) {
        depth.hash(&mut state.cfg_hasher);
        normalize(kind).hash(&mut state.cfg_hasher);
    }
}

fn visit_children(state: &mut FingerprintState, node: Node, source: &[u8], depth: usize) {
    (node.child_count() as u64).hash(&mut state.struct_hasher);
    for (i, child) in node.children(&mut node.walk()).enumerate() {
        (i as u64).hash(&mut state.struct_hasher);
        state.visit(child, source, depth + 1);
    }
}
