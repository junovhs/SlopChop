...
use warden_core::apply::types::{ManifestEntry, Operation, FileContent};
...
fn test_timestamp_folder() -> Result<()> {
    ...
    assert!(!entries.is_empty(), "Should have created a timestamp directory");
    
    let first_entry = entries[0].as_ref().map_err(std::string::ToString::to_string)?;
...
fn test_existing_backed_up() -> Result<()> {
    ...
    files.insert(file_path.to_string(), FileContent { 
    ...
    let backup_root = d.path().join(".warden_apply_backup");
    let timestamp_dir = fs::read_dir(backup_root)?.next().ok_or("No backup dir")??.path();
...
fn test_path_structure() -> Result<()> {
    ...
    let backup_root = d.path().join(".warden_apply_backup");
    let timestamp_dir = fs::read_dir(backup_root)?.next().ok_or("No backup")??.path();
