...
fn setup_test_project() -> TempDir {
    ...
    fs::write(temp.path().join("slopchop.toml"), "[rules]\nmax_file_tokens = 2000\n").unwrap();
    fs::write(temp.path().join("tasks.toml"), r#"
[meta]
title = "Test"
description = ""
...
"#).unwrap();
    ...
    fs::write(temp.path().join("tests/unit.rs"), "#[test]\nfn test_fn() {}\n").unwrap();
    temp
}

#[test]
fn test_roadmap_init() {
    let temp = TempDir::new().unwrap();
...
#[test]
fn test_roadmap_tasks() {
    let temp = setup_test_project();
    let content = fs::read_to_string(temp.path().join("tasks.toml")).unwrap();
    assert!(content.contains("test-task"));
}

#[test]
fn test_roadmap_tasks_pending() {
    let temp = setup_test_project();
    let content = fs::read_to_string(temp.path().join("tasks.toml")).unwrap();
...
#[test]
fn test_roadmap_tasks_complete() {
    let temp = setup_test_project();
    let content = fs::read_to_string(temp.path().join("tasks.toml")).unwrap();
